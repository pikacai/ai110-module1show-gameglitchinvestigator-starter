...
I used Claude Code as an in-editor agent that could read the files, edit them, run pytest, and launch the app. A correct suggestion: it traced the "Too low for 26" bug to the `str(secret)` conversion forcing string comparison, and I verified it with a quick script (`check_guess(26, 5)` → "Too High" → "Go LOWER") and by playing the live app. A misleading moment: after the first round of fixes the app *still* showed the old wrong hint — the AI initially assumed the code was wrong again, but the real cause was a stale Streamlit process serving cached code; killing all `streamlit run` processes and restarting fixed it. That taught me to confirm *which* process and code version I'm actually looking at before assuming the logic is broken. I also had to *reject* a suggestion: when fixing the guess box that wouldn't clear after **New Game**, the AI's first instinct was to set `st.session_state["guess_input_..."] = ""` inside the New Game block. I pushed back because that key's widget is created earlier in the same run, so Streamlit throws "cannot modify a widget after it is instantiated." We instead bumped an `input_round` counter that changes the widget key, which I confirmed by adding `test_new_game_changes_guess_input_key_so_box_clears` and by clicking New Game in the live app and seeing the box empty.
...
The habit I want to keep is marking the "crime scene" with `# FIXME` comments and writing a failing test *before* fixing, then confirming both the test and the live app afterward. Next time I'd restart the dev server (or check the running process) earlier, since I wasted time thinking a fix hadn't worked when I was really looking at a stale instance. Overall this project made me treat AI-generated code as a confident first draft that needs verification: it looked production-ready and ran without errors, yet had five real bugs that only surfaced through testing and actually playing the game.
